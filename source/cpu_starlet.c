...
void ARM_print_state() {
    // hack to just put this here now
    EXI_FlushGeckoText();
...
static char debug_out_buffer[0x1000] = { 0 };
...
void ARM_intr_hook(uc_engine *uc, uint32_t intno, void *user_data) {
    ...
    if (intno == 2) { // SWI
        ...
        if (r0 == 4) { // IOS uses this for debugger output
            ...
            if (debug_out_buffer[strlen(debug_out_buffer) - 1] == '\n') {
                ...
                ARM_printfv("Debug: %s", debug_out_buffer);
    ...
    if (intno == 3) {
        ARM_printfv("Prefetch abort at %08x", pc);
        uc_emu_stop(uc);
    }
...
int ARM_Main() {
    ...
    // map EXI registers
    uc_mmio_map(ARM_unicorn, REG_EXI | REG_AHBMASK, ALIGN_SIZE(REG_EXI_SZ), EXI_ReadRegister, NULL, EXI_WriteRegister, NULL);
    ...
    fclose(mem1dump);
    // dump the contents of MEM2
    FILE *mem2dump = fopen("mem2.bin", "w+");
    fwrite(MEM2_Buffer, 1, MEM2_SIZE, mem2dump);
    fclose(mem2dump);
